...
export default function StickerManager() {
  ...
  // Agrupa por time e aplica filtro
  const byTeam = useMemo(() => {
    const map = {};
    stickers.forEach(s => {
      if (!map[s.team_code]) map[s.team_code] = { name: s.team_name, code: s.team_code, items: [] };
  ...
  const addSticker = async () => {
    ...
    try {
      ...
      const novos  = results.filter(d => d.owned_count === 1).length;
      const repets = results.filter(d => d.owned_count > 1).length;
      let msg = '';
      if (novos)         msg += `${novos} colada${novos > 1 ? 's' : ''} no album`;
      if (repets)        msg += `${msg ? ' • ' : ''}${repets} repetida${repets > 1 ? 's' : ''}`;
      if (errors.length) msg += `${msg ? ' • ' : ''}Invalido: ${errors.map(e => e.code).join(', ')}`;
      flash(errors.length && !novos && !repets ? 'err' : 'ok', msg || 'OK');
      setInput('');
  ...
  const FILTERS = [
    { id:'todas',     label:'Todas',     count: stickers.length },
    { id:'coladas',   label:'Coladas',   count: stats.coladas   },
    { id:'repetidas', label:'Repetidas', count: stats.repetidas },
  ];
    ...
    <div style={{ minHeight:'100vh',background:'#060d18',fontFamily:"'Segoe UI',sans-serif",color:'#e2e8f0',paddingBottom:40 }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0f2744,#0a1e38)',borderBottom:'2px solid #d4a017',padding:'14px 14px 0' }}>
        ...
        <div style={{ fontSize:11,color:'#64748b',marginBottom:10 }}>
          Ola, <strong style={{ color:'#e2e8f0' }}>{user.name}</strong>
        </div>
        ...
        <div style={{ display:'flex',borderTop:'1px solid #1e3a5c' }}>
          {FILTERS.map(f => (
            <button key={f.id} onClick={() => setFilter(f.id)} style={{
              flex:1,padding:'9px 4px 11px',background:'none',border:'none',
              borderBottom: filter===f.id ? '3px solid #d4a017' : '3px solid transparent',
              color: filter===f.id ? '#d4a017' : '#64748b',
              cursor:'pointer',fontSize:11,fontWeight:700,
              display:'flex',flexDirection:'column',alignItems:'center',gap:3,
            }}>
              <span>{f.label}</span>
              <span style={{ background:filter===f.id?'#d4a017':'#1e3a5c',color:filter===f.id?'#060d18':'#94a3b8',borderRadius:20,padding:'1px 7px',fontSize:9,fontWeight:900 }}>
                {f.count}
              </span>
            </button>
          ))}
          ...
          {byTeam.length === 0 && (
            <div style={{ textAlign:'center',padding:'50px 20px',color:'#475569' }}>
              <div style={{ fontSize:44,marginBottom:12 }}>
                {filter==='repetidas' ? '🔄' : filter==='coladas' ? '📗' : '📖'}
              </div>
              <p>
                {filter==='repetidas' ? 'Nenhuma repetida ainda.' :
                 filter==='coladas'   ? 'Nenhuma figurinha colada ainda.' :
                ...
                {items.map(item => {
                  ...
                  return (
                    <div key={item.code} style={{
                      background: isRep ? 'rgba(212,160,23,0.12)' : 'rgba(74,222,128,0.08)',
                      border:`1px solid ${isRep?'rgba(212,160,23,0.5)':'rgba(74,222,128,0.3)'}`,
                      borderRadius:8,padding:'5px 9px',display:'flex',alignItems:'center',gap:5,
                    }}>
                      <span style={{ fontWeight:700,color:isRep?'#d4a017':'#4ade80',fontSize:12 }}>
                        {code} {item.number}
                      </span>
                      {isRep && (
                        <span style={{ background:'#d4a017',color:'#060d18',borderRadius:20,padding:'1px 6px',fontSize:9,fontWeight:900 }}>
                          x{item.owned_count-1} p/troca
                        </span>
                      )}
